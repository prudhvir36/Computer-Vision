...
for c in dr:
    ...
    faces = face_cascade.detectMultiScale(
        gray, scaleFactor=1.2, minNeighbors=10)
    ...
    for (x_face, y_face, w_face, h_face) in faces:
        ...
        cv2.rectangle(frame,
                      pt1=(x_face-int(np.floor(dw)),
                           (y_face-int(np.floor(dh)))),
                      pt2=((x_face+w_face+int(np.ceil(dw))),
                           (y_face+h_face+int(np.ceil(dh)))),
                      color=(0, 255, 0),
                      thickness=2)  # Face Rectangle

        cv2.rectangle(frame,
                      pt1=(x_face-int(np.floor(dw)),
                           y_face-int(np.floor(dh))-size[1]),
                      pt2=(x_face-int(np.floor(dw)) +
                           size[0], y_face-int(np.floor(dh))),
                      color=(0, 255, 0),
                      thickness=-1)  # Face Name background rectangle

        img = Image.fromarray(frame)
        draw = ImageDraw.Draw(img)
        draw.text((x_face-int(np.floor(dw)), y_face-int(np.floor(dh)) -
                   size[1]), name, font=font, fill=(255, 0, 0))
        frame = np.array(img)
