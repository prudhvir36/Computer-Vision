...
from meghnad.core.cv.obj_det.src.dt.trn.trn import DT2ObjDetTrn
...
class Trainer:
    def __init__(self, settings: List[str]) -> None:
        ...
        for model_cfg in self.model_cfgs:
            ...
            elif backend == BACKENDS.DETECTRON2:
                dt2_model_cfgs.append(model_cfg)
            else:
        ...
        self.dt2_trainer = DT2ObjDetTrn(dt2_model_cfgs)
    ...
    def trn(self,
        ...
        success, tf_best_metric, tf_best_path = self.tf_trainer.trn(
            epochs=epochs,
            hyp=kwargs.get('hyp_params', dict()),
            output_dir=tf_output_dir
        )
        del self.tf_trainer

        dt2_output_dir = os.path.join(output_dir, 'dt2')
        self.dt2_trainer.config_connectors(self.data_path, self.augmentations)
        success, dt_best_metric, dt_best_path = self.dt2_trainer.trn(
            batch_size=batch_size,
            epochs=epochs,
            workers=workers,
            device=device,
            hyp=kwargs.get('hyp', dict()),
            output_dir=dt2_output_dir
        )
        del self.dt2_trainer

        # compare
        best_metric = None
        best_metrics_lst = []

        best_metrics_lst.append(pt_best_metric.map)
        best_metrics_lst.append(tf_best_metric.map)
        best_metrics_lst.append(dt_best_metric.map)

        best_metric_value = max(best_metrics_lst)

        if dt_best_metric.map == best_metric_value:
            best_metric = dt_best_metric
            best_model_path = dt_best_path
        elif pt_best_metric.map == best_metric_value:
            best_metric = pt_best_metric
            best_model_path = pt_best_path
        elif tf_best_metric.map == best_metric_value:
