...
_defaults = _config.get('default_constants')['tf']['trn']['eval']
...
class TFObjDetEval:
    ...
    def eval(self,
             ...
             class_map: Dict = _defaults['class_map'],
             score_threshold: float = _defaults['score_threshold'],
             nms_threshold: float = _defaults['nms_threshold'],
             max_predictions: int = _defaults['max_predictions'],
             image_out_dir: str = _defaults['image_out_dir'],
             draw_predictions: bool = _defaults['draw_predictions']) -> Tuple[Any]:
        ...
        if phase == 'validation':
            ...
            log.ERROR(sys._getframe().f_lineno,
                      __file__, __name__, f"Not found ground truth annotation file")
            return ret_values.IXO_RET_GENERIC_FAILURE
        ...
        if os.path.isfile(pred_file):
            os.remove(pred_file)
        return ret_values.IXO_RET_SUCCESS, map, map50
