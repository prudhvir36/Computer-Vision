...
def sparse_categorical_focal_loss(
    ...
    if y_pred_rank is not None:
        ...
        log.ERROR(sys._getframe().f_lineno,
                  __file__, __name__, f'Cannot compute sparse categorical focal loss with axis={axis} on '
            'a prediction tensor with statically unknown rank.')
        return ret_values.IXO_RET_EXTERNAL_LIB_ERROR
    ...
    if y_true_rank is None:
        log.ERROR(sys._getframe().f_lineno,
                  __file__, __name__, f'Sparse categorical focal loss not supported '
                                       'for target/label tensors of unknown rank')
        return ret_values.IXO_RET_NOT_IMPLEMENTED
...
@method_header(
    ...
    """)
def _hard_negative_mining(loss: tf.Tensor, gt_confs: tf.Tensor, neg_ratio: float):
    pos_idx = gt_confs > 0
...
class Loss(object):
    ...
    def __call__(self,
        ...
        temp_loss = cross_entropy(
            gt_confs, confs)
        pos_idx, neg_idx = _hard_negative_mining(
        ...
        return ret_values.IXO_RET_SUCCESS, conf_loss, loc_loss
