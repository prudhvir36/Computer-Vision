...
@method_header(
    ...
    """)
def _compute_area(top_left: tf.Tensor, bot_right: tf.Tensor) -> tf.Tensor:
...
@method_header(
    ...
    """)
def _compute_iou(boxes_a: tf.Tensor, boxes_b: tf.Tensor) -> tf.Tensor:
    ...
    overlap_area = _compute_area(top_left, bot_right)
    area_a = _compute_area(boxes_a[..., :2], boxes_a[..., 2:])
    area_b = _compute_area(boxes_b[..., :2], boxes_b[..., 2:])
...
def compute_target(
        ...
        gt_boxes: tf.Tensor,
        gt_labels: tf.Tensor,
        iou_threshold: bool = 0.5) -> Tuple[tf.Tensor]:

    # Convert default boxes to format (xmin, ymin, xmax, ymax)
    # in order to compute overlap with gt boxes
    transformed_default_boxes = _transform_center_to_corner(default_boxes)
    iou = _compute_iou(transformed_default_boxes, gt_boxes)
    ...
    gt_locs = _encode(default_boxes, gt_boxes)
...
def _encode(
        default_boxes: tf.Tensor,
        boxes: tf.Tensor,
        variance: Tuple[float] = [0.1, 0.2]) -> tf.Tensor:

    # Convert boxes to (cx, cy, w, h) format
    transformed_boxes = _transform_corner_to_center(boxes)
...
def decode(
    ...
    boxes = _transform_center_to_corner(locs)
...
@method_header(
    ...
    Returns:
        boxes: tensor (num_boxes, 4)
               of format (cx, cy, w, h)
    """)
def _transform_corner_to_center(boxes: tf.Tensor) -> tf.Tensor:
...
@method_header(
    ...
    Returns:
        boxes: tensor (num_boxes, 4)
               of format (xmin, ymin, xmax, ymax)
    """)
def _transform_center_to_corner(boxes: tf.Tensor) -> tf.Tensor:
