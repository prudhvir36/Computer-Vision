...
@class_header(
    description="""Class for Box Objects Representation
              """) 
class Box:
    ...
    @method_header(description=
                   "Serializes and returns a dictionary")
    def to_dict(self):
        box = OrderedDict([
...
@class_header(description=
              """Represents a set of object detections with their 
              respective bounding boxes, confidence scores, and classes""")
class Detections:
    ...
    @method_header(
            description="""Extracts the bounding boxes, confidence scores, 
            class names, and track IDs (if tracking is enabled) from the raw detection 
            data.""")
    ...
    @method_header(
            description="""Returns a list of Box objects representing 
        the bounding boxes of the object detections.
        """)
    def get_boxes(self):
        return self.__boxes

    @method_header(
            description="""Returns a dictionary representation of the Detections object.
    """)
    def to_dict(self):
        boxes = []
    ...
    @method_header(
            description="""Returns a JSON representation of the Detections object.
            """)
    def to_json(self):
        boxes = self.to_dict()
...
@method_header(
        description="""draws a rectangle with label and fills it with 
            color inside a given bounding box on an image.
            """)
def plot_box(image, 
             top_left_point, 
             bottom_right_point, 
    ...
    point3 = top_left_point['x'] + font_width + padding*2, top_left_point['y'] - font_height - padding*2
    cv2.rectangle(image, (top_left_point['x'] - 2, top_left_point['y']), point3, color, -1, lineType=cv2.LINE_AA)
...
@method_header(
        description="""draws boxes around the detected objects with labels 
